...
def inference_meta_epoch(c, epoch, loader, extractor, parallel_flows, fusion_flow):
    ...
    with torch.no_grad():
        ...
        #for idx, (image, label, mask) in enumerate(loader):
            ...
            z_list, jac = model_forward(c, extractor, parallel_flows, fusion_flow, image)
###########################################################################################TODO   
#注意：每个类输入尺寸不一样   #jac单个数值   z_list（8,256,64,64）（8,512,32,32）（8,1024,16,16）  
            batch_size = image.size(0)
            for i in range(batch_size):
                # 计算在test_dataset中的索引
                sample_idx = idx * loader.batch_size + i
                file_path = file_paths[sample_idx]
                
                # folder_path = file_path.replace(
                #     '/mnt/T4_2/xjj/FirstWorkData1/', 
                #     '/mnt/T4_2/xjj/msflow/FirstWorkData_numpy/'
                # ).rsplit('.', 1)[0]
                folder_path = file_path.replace(
                    '/mnt/T38/bioinf/xjj/Datasets/MVTec-AD/', 
                    '/mnt/T4_1/xjj/msflow/MVTec-AD_numpy/'
                ).rsplit('.', 1)[0]
                
                os.makedirs(folder_path, exist_ok=True)
                
                np.save(os.path.join(folder_path, f"z_64x64.npy"), z_list[0][i].cpu().detach().numpy())
                np.save(os.path.join(folder_path, f"z_32x32.npy"), z_list[1][i].cpu().detach().numpy())
                np.save(os.path.join(folder_path, f"z_16x16.npy"), z_list[2][i].cpu().detach().numpy())
                
                paths.append(file_path)
                jac_values.append(jac[i].item())
        
            df = pd.DataFrame(list(zip(paths, jac_values)), columns=['path', 'jac'])
            # os.makedirs('/mnt/T4_2/xjj/msflow/FirstWorkData_numpy/', exist_ok=True)
            # df.to_csv('/mnt/T4_2/xjj/msflow/FirstWorkData_numpy/jac_values.csv', index=False)
            os.makedirs('/mnt/T4_1/xjj/msflow/MVTec-AD_numpy/', exist_ok=True)
            df.to_csv('/mnt/T4_1/xjj/msflow/MVTec-AD_numpy/jac_values.csv', index=False)
###########################################################################################
            loss = 0.
