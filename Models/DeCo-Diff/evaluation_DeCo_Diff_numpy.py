...
def evaluate_anomaly_maps(anomaly_maps, segmentation, category=None):
    """
    Evaluate anomaly maps and print results.
    
    Args:
        anomaly_maps: Dictionary of anomaly maps
        segmentation: Ground truth segmentation masks
        category: Category name for printing
    ...
    for key in anomaly_maps.keys():
        ...
        print(' aurocsp:{:.4f}'.format(key, aurocsp))
        ...
        results[key] = {
            ...
            'aupro': float(aupro_score),
            'f1_max': float(f1_max_score),
            'ap': float(ap),
            'aurocsp': float(aurocsp),
            'apsp': float(apsp),
            'f1sp': float(f1sp)
        }
...
def evaluation(args):
    ...
    with open(summary_filepath, 'w') as summary_file:
        ...
        summary_file.write(f"Model Size: {args.model_size}\n")
        summary_file.write(f"Object Category: {args.object_category}\n")
        summary_file.write(f"Image Size: {args.image_size}\n")
        summary_file.write(f"Center Size: {args.center_size}\n")
        summary_file.write(f"Center Crop: {args.center_crop}\n")
        summary_file.write(f"VAE Type: {args.vae_type}\n")
        summary_file.write(f"Reverse Steps: {args.reverse_steps}\n")
        summary_file.write("=" * 60 + "\n\n")
    ...
    for category in args.categories:
        ...
        for ii, (x, seg, object_cls, img_paths) in enumerate(test_loader):
            ...
            image_paths_s += list(img_paths)

        print(category)        
###########################################################################################################################TODO
#x0_s(1,3,256,256)   encoded_s(1,4,32,32)     image_samples_s(1,3,256,256)   latent_samples_s(1,4,32,32) 
        # 获取批次中的所有图像路径
        # images_paths 应该是包含批次中所有图像路径的列表
        for batch_idx, img_path_str in enumerate(image_paths_s):
            # 构建文件夹路径
            
            folder_path = img_path_str.replace(
                '/mnt/T4_2/xjj/FirstWorkData1/', 
                '/mnt/T4_1/xjj/DeCo-Diff/FirstWorkData_numpy/'
            ).rsplit('.', 1)[0]

            # folder_path = img_path_str.replace(
            #     '/mnt/T38/bioinf/xjj/Datasets/MVTec-AD/', 
            #     '/mnt/T4_1/xjj/DeCo-Diff/MVTec-AD_numpy/'
            # ).rsplit('.', 1)[0]

            # 创建文件夹
            os.makedirs(folder_path, exist_ok=True)
            
            # 保存 x0_s, encoded_s, image_samples_s, latent_samples_s
            np.save(os.path.join(folder_path, f"x0_s.npy"), x0_s[batch_idx].cpu().detach().numpy())
            np.save(os.path.join(folder_path, f"encoded_s.npy"), encoded_s[batch_idx].cpu().detach().numpy())
            np.save(os.path.join(folder_path, f"image_samples_s.npy"), image_samples_s[batch_idx].cpu().detach().numpy())
            np.save(os.path.join(folder_path, f"latent_samples_s.npy"), latent_samples_s[batch_idx].cpu().detach().numpy())
###########################################################################################################################
        '''
        anomaly_maps = calculate_anomaly_maps(x0_s, encoded_s,  image_samples_s, latent_samples_s, center_size=args.center_size)
        # Evaluate and save results for this category
        category_results = evaluate_anomaly_maps(
            anomaly_maps, 
        ...
        # Append to summary file in compact format (same as terminal printing)
        with open(summary_filepath, 'a') as summary_file:
            summary_file.write(f"{category}\n")
            for key, metrics_dict in category_results.items():
                summary_file.write('{}: auroc:{:.4f}, aupro:{:.4f}, f1_max:{:.4f}, ap:{:.4f}, aurocsp:{:.4f}, apsp:{:.4f}, f1sp:{:.4f}\n'.format(
                    key, metrics_dict['auroc'], metrics_dict['aupro'], metrics_dict['f1_max'], 
                    metrics_dict['ap'], metrics_dict['aurocsp'], metrics_dict['apsp'], metrics_dict['f1sp']
                ))
            summary_file.write("\n")
        '''
        print('=='*30)
