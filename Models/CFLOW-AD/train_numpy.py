...
def test_meta_epoch(c, epoch, loader, encoder, decoders, pool_layers, N):
    ...
    with torch.no_grad():
        ...
        for i, (image, label, original_size) in enumerate(tqdm(loader, disable=c.hide_tqdm_bar)):
            ...
            #gt_mask_list.extend(t2np(mask))   #注释
            ...
            for l, layer in enumerate(pool_layers):
                ...
                if 'cflow' in c.dec_arch:
                    ...
                    z, log_jac_det = decoder(e_p)             
########################################################################################TODO  z:([131072, 512])   log_jac_det:([131072])   注意：每个类输入尺寸不一样

                # ===== 添加：将z和log_jac_det还原回每张图片的特征图形式 =====
                # z当前形状: [E, C], 其中 E = B * S, S = H * W
                # log_jac_det当前形状: [E]
                # 逆向变换:
                # 1. z: reshape(B, S, C) -> transpose(1, 2) -> reshape(B, C, H, W)
                # 2. log_jac_det: reshape(B, H, W)
                z_reshaped = z.reshape(B, S, C).transpose(1, 2).reshape(B, C, H, W)
                log_jac_det_reshaped = log_jac_det.reshape(B, H, W)
                # 打印还原后的形状进行验证
                #print(f"【DEBUG】z_reshaped shape: {z_reshaped.shape}")
                #zipper（128,512,64,64）（128,1024,32,32）（128,2048,16,16）
                #print(f"【DEBUG】log_jac_det_reshaped shape: {log_jac_det_reshaped.shape}")
                #zipper（128,64,64）（128,32,32）（128,16,16）
# ===============================================================================================
                # 保存z_reshaped和log_jac_det_reshaped
                # 注意：这里是每个batch（128张图片）中每张图片都要保存
                # img_paths是原始路径，我们需要根据当前batch的索引来获取对应图片的路径
                
                # 计算当前batch在img_paths中的起始索引
                fixed_batch_size = c.batch_size
                batch_start_idx = i * fixed_batch_size
                
                for batch_idx in range(B):
                    # 获取当前图片在完整数据集中的索引
                    img_idx = batch_start_idx + batch_idx
                    # 确保索引不超出img_paths的范围
                    if img_idx < len(img_paths):
                        img_path_str = img_paths[img_idx]
                        
                        # # 构建文件夹路径
                        folder_path = img_path_str.replace(
                            '/mnt/T4_2/xjj/FirstWorkData1/', 
                            '/mnt/T4_1/xjj/cflow-ad/FirstWorkData_numpy/'
                        ).rsplit('.', 1)[0]
                        
                        # 构建文件夹路径
                        # folder_path = img_path_str.replace(
                        #     '/mnt/T38/bioinf/xjj/Datasets/MVTec-AD/', 
                        #     '/mnt/T4_1/xjj/cflow-ad/MVTec-AD_numpy/'
                        # ).rsplit('.', 1)[0]

                        os.makedirs(folder_path, exist_ok=True)
                        
                        # 根据尺寸命名文件
                        # layer 0: (128,512,64,64) 和 (128,64,64)
                        # layer 1: (128,1024,32,32) 和 (128,32,32)
                        # layer 2: (128,2048,16,16) 和 (128,16,16)
                        
                        # 提取当前图片的z_reshaped和log_jac_det_reshaped
                        z_single = z_reshaped[batch_idx].cpu().detach().numpy()  # (C, H, W)
                        log_jac_det_single = log_jac_det_reshaped[batch_idx].cpu().detach().numpy()  # (H, W)
                        
                        # 保存文件，文件名包含尺寸信息
                        np.save(os.path.join(folder_path, f"z_reshaped_layer{l}_{H}x{W}.npy"), z_single)
                        np.save(os.path.join(folder_path, f"log_jac_det_reshaped_layer{l}_{H}x{W}.npy"), log_jac_det_single)
########################################################################################################################################
...
def train(c):
    ...
    for epoch in range(c.meta_epochs):
        ...
        # score aggregation
        #score_map = np.zeros_like(test_map[0])    #注释
        #注释
        # for l, p in enumerate(pool_layers):
