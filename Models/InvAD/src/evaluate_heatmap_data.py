...
def main(config, args):
    ...
    if is_multi_class:
        ...
        avg_img_auc,avg_px_auc,avg_img_ap,avg_px_ap,avg_pro,avg_img_f1,avg_px_f1,avg_latency,avg_memory_usage = 0,0,0,0,0,0,0,0,0,
...
def evaluate_inv(denoiser, feature_extractor, anom_loaders, normal_loaders, config, in_sh, epoch, eval_step, device, args):
    ...
    for normal_loader, anom_loader in zip(normal_loaders, anom_loaders):
        ...
        diffs_min = np.min([normal_diffs.min()])
        diffs_max = np.max([normal_diffs.max()])
        nlls_min = np.min([normal_nlls.min()])
        nlls_max = np.max([normal_nlls.max()])
        ...
        #y_score = np.concatenate([normal_scores, anomaly_scores])
        y_score = np.concatenate([normal_scores])
        # img_metrics = calculate_img_metrics(
        #     gt_labels=y_true,
        #     pred_scores=y_score,
        #     metrics=['img_auroc', 'img_ap', 'img_f1max', 'img_ap'],
        # )
        ...
        px_metrics = calculate_px_metrics(
            ...
            metrics=['px_auroc', 'px_ap', 'px_f1max', 'px_ap', 'px_aupro']
        )
        ...
        '''
        # 如果像素级指标计算被注释，设置默认值避免KeyError
        if 'P-AUROC' not in metrics_dict[category]:
            metrics_dict[category]["P-AUROC"] = 0.0
            metrics_dict[category]["P-AP"] = 0.0
            metrics_dict[category]["PRO"] = 0.0
            metrics_dict[category]["P-F1Max"] = 0.0
            img_f1max=img_ap=img_auroc=px_auroc = px_ap = px_pro = px_f1max = 0.0
        ...
        # -------------------- 保存热力图和预测分数 -------------------- 
        save_path = "/mnt/T4_1/xjj/2/Invad/FirstWorkData1" 
        os.makedirs(save_path, exist_ok=True) 
        
        # 合并所有异常图（正常+异常样本）
        all_anomaly_maps = np.concatenate([normal_maps], axis=0)
        
        # 将异常图转换为张量 
        prediction_tensor = torch.from_numpy(all_anomaly_maps)
        ...
        for batch in normal_loader:
            if 'filenames' in batch:
                all_image_paths.extend(batch['filenames'])
        ...
        # 如果仍然没有获取到路径，尝试从数据集中获取
        if not all_image_paths:
            logger.info("尝试从数据集中获取图像路径...")
            try:
                # 从正常数据集获取路径
                if hasattr(normal_loader.dataset, 'samples'):
                    for sample in normal_loader.dataset.samples:
                        if isinstance(sample, (list, tuple)) and len(sample) > 0:
                            all_image_paths.append(sample[0])
                # 从异常数据集获取路径
                if hasattr(anom_loader.dataset, 'samples'):
                    for sample in anom_loader.dataset.samples:
                        if isinstance(sample, (list, tuple)) and len(sample) > 0:
                            all_image_paths.append(sample[0])
            except Exception as e:
                logger.warning(f"从数据集获取路径失败: {e}")
        
        # 确保category不为None
        if category is None:
            category = anom_loader.dataset.object_class if hasattr(anom_loader.dataset, 'object_class') else "unknown"
        
        # 保存热力图（如果有图像路径） 
        if all_image_paths: 
            save_heatmaps(prediction_tensor, all_image_paths, save_path, category) 
