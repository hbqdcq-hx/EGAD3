...
class MVTecDataset(Dataset):
    def __init__(
        ...
        if is_train:
            self.mvtec_paths = sorted(glob.glob(mvtec_dir + "/*.jpg"))
            self.dtd_paths = sorted(glob.glob(dtd_dir + "/*/*.jpg"))
            self.rotate_90 = rotate_90
            self.random_rotate = random_rotate
        else:
            #self.mvtec_paths = sorted(glob.glob(mvtec_dir + "/*/*.jpg"))
            self.mvtec_paths = sorted(glob.glob(mvtec_dir + "/*/*.jpg") + glob.glob(mvtec_dir + "/*/*.png"))
            self.imageDir = self.mvtec_paths

            print(len(self.mvtec_paths))
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            return {"img": image, "mask": mask, "file_path": self.mvtec_paths[index]}
