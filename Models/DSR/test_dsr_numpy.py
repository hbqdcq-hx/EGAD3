...
def evaluate_model(model, model_normal, model_normal_top, model_decode, decoder_seg, model_upsample, obj_name, mvtec_path, cnt_total):
    ...
    for i_batch, sample_batched in enumerate(dataloader):
        ...
        out_mask = decoder_seg(recon_image_recon.detach(),
                               recon_image.detach())
        ######################################################################################################################TODO   out_mask(1,2,256,256) upsampled_mask(1,2,256,256)
        # 获取当前图像的路径（使用sample_batched中的idx字段，这是数据集中的原始索引）
        img_idx = sample_batched["idx"].item()  # 从数据集中获取原始索引
        img_path_str = dataset.images[img_idx]
        # 构建npy文件保存路径
        npy_path = img_path_str.replace(
            '/mnt/T4_2/xjj/FirstWorkData1/', 
            '/mnt/T4_1/xjj/DSR/FirstWorkData_numpy/'
        ).rsplit('.', 1)[0] + '.npy'
        # npy_path = img_path_str.replace(
        #             '/mnt/T38/bioinf/xjj/Datasets/MVTec-AD/', 
        #             '/mnt/T4_2/xjj/DSR/MVTec-AD_numpy/'
        #         ).rsplit('.', 1)[0] + '.npy'
        ...
        # 保存upsampled_mask特征
        # upsampled_mask形状: (1, 2, 256, 256)，转换为numpy并保存
        out_mask_feat_np = out_mask.detach().cpu().numpy()  # (1, 2, 256, 256)
        np.save(npy_path, out_mask_feat_np)
        #print(f"Saved upsampled_mask feature to: {npy_path}")  # 可选：打印保存路径
######################################################################################################################
        ...
        out_mask_sm_up = crop_image(out_mask_sm_up, img_dim)       #（1,2,224,224）

        iter += 1

        out_mask_cv = out_mask_sm_up[0,1,:,:].detach().cpu().numpy()  #丢掉了通道0（应该是背景）
