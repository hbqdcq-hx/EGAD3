...
def evaluation_indusAD(c, model, dataloader, device):
    ...
    with torch.no_grad():
        ...
        for idx, (sample, images_paths) in enumerate(dataloader):
            #注释
...
########################################################################################################TODO  t是source+target,s是target+source
...
# torch.Size([1, 1024,16, 16])
            ...
            for batch_idx, img_path_str in enumerate(images_paths):
                # 构建文件夹路径
                folder_path = img_path_str.replace(
                    '/mnt/T38/bioinf/xjj/Datasets/MVTec-AD/', 
                    '/mnt/T4_2/xjj/UniNet/MVTec-AD_numpy/'
                ).rsplit('.', 1)[0]
                
                # 创建文件夹
                os.makedirs(folder_path, exist_ok=True)
                
                # 保存 t_tf 特征图 (前三个是 source_t，后三个是 target_t)
                np.save(os.path.join(folder_path, f"source_t_64x64.npy"), t_tf[0][batch_idx].cpu().detach().numpy())
                np.save(os.path.join(folder_path, f"source_t_32x32.npy"), t_tf[1][batch_idx].cpu().detach().numpy())
                np.save(os.path.join(folder_path, f"source_t_16x16.npy"), t_tf[2][batch_idx].cpu().detach().numpy())
                
                np.save(os.path.join(folder_path, f"target_t_64x64.npy"), t_tf[3][batch_idx].cpu().detach().numpy())
                np.save(os.path.join(folder_path, f"target_t_32x32.npy"), t_tf[4][batch_idx].cpu().detach().numpy())
                np.save(os.path.join(folder_path, f"target_t_16x16.npy"), t_tf[5][batch_idx].cpu().detach().numpy())
                
                # 保存 de_features 特征图 (前三个是 target_s，后三个是 source_s)
                np.save(os.path.join(folder_path, f"target_s_64x64.npy"), de_features[0][batch_idx].cpu().detach().numpy())
                np.save(os.path.join(folder_path, f"target_s_32x32.npy"), de_features[1][batch_idx].cpu().detach().numpy())
                np.save(os.path.join(folder_path, f"target_s_16x16.npy"), de_features[2][batch_idx].cpu().detach().numpy())
                
                np.save(os.path.join(folder_path, f"source_s_64x64.npy"), de_features[3][batch_idx].cpu().detach().numpy())
                np.save(os.path.join(folder_path, f"source_s_32x32.npy"), de_features[4][batch_idx].cpu().detach().numpy())
                np.save(os.path.join(folder_path, f"source_s_16x16.npy"), de_features[5][batch_idx].cpu().detach().numpy())
##################################################################################################
    ...
    return auroc_px, auroc_sp, pro, anomaly_score, anomaly_map
