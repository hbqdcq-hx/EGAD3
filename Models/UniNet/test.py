...
from eval_heatmap import evaluation_indusAD, evaluation_batch, evaluation_mediAD, evaluation_polypseg, \
    ...
    if c.domain == 'industrial':
        if c.setting == 'oc':
            if dataset_name in unsupervised:
                auroc_px, auroc_sp, pro, anomaly_score, anomaly_map = evaluation_indusAD(c, model, test_dataloader, device)
                return auroc_sp, auroc_px, pro, anomaly_score, anomaly_map
