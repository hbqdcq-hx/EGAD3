...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        # self.conv_ = nn.Conv2d(256, 256, kernel_size=1, stride=1, padding=0, bias=True)


        # self.basic = self.Basic(256, 256)

        # sg
        self.conv1x1_1 = nn.Conv2d(256, 1, 1, 1, 0, bias=False)
        # self.conv1x1_2 = nn.Conv2d(512, 1, 1, 1, 0, bias=False)
        # self.conv1x1_3 = nn.Conv2d(1024, 1, 1, 1, 0, bias=False)

        # self.moduleDeconv4 = self.Basic(512, 256)
        ...
        # self.generator = self.Generator(32, 1, 32)
    ...
    def _forward_impl(self, x: Tensor, skip=None) -> Tensor:
        ...
        feature_b = self.layer2(feature_a + skip[0])  # 512*32*32
        feature_c = self.layer3(feature_b + skip[1])  # 256*64*64
        # feature_d = self.layer4(feature_c)  # 128*128*128

        # feature_c_ = torch.split(feature_c, [8, 8], dim=0)
        ...
        # feature_b_ = self.conv_(channel_shuffle(self.up1(feature_b), 256))
        # feature_c_ = self.basic(feature_b_ + feature_c)
        # feature_a_ = self.conv1x1_3(feature_a)
        # feature_b_ = self.conv1x1_2(feature_b)
        feature_c_ = self.conv1x1_1(feature_c)

        # fb = self.up3(feature_b)
        # de_f3 = self.moduleDeconv3(feature_c + fb)
        # up2 = self.up2(de_f3)
        #
        ...
        return [feature_c, feature_b, feature_a], [feature_c_, ]
