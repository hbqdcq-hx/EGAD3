    ...
    def test(self, training_data, test_data, save_segmentation_images):
        ...
        if os.path.exists(ckpt_path):
            ...
            if 'discriminator' in state_dict:
                ...
                # 如果权重文件不包含discriminator键，可能是旧格式
                self.load_state_dict(state_dict, strict=False)

        # 运行预测
        scores, segmentations, features, labels_gt, masks_gt, output = self.predict(test_data)
####################################################################################################TODO  scores:单个数值   segmentations（N，288,288） features（1536,288,288）
      
        # 获取当前批次的图像路径
        image_paths = [x[2] for x in test_data.dataset.data_to_iterate]
        
        # 遍历批次中的每个图像
        for i, img_path_str in enumerate(image_paths):
            # 构建npy文件保存路径
            npy_path = img_path_str.replace(
                '/mnt/T4_2/xjj/FirstWorkData1/', 
                '/mnt/T4_1/xjj/SimpleNet/FirstWorkData_numpy/'
            ).rsplit('.', 1)[0] + '.npy'

            # npy_path = img_path_str.replace(
            #     '/mnt/T38/bioinf/xjj/Datasets/MVTec-AD', 
            #     '/mnt/T4_1/xjj/SimpleNet/MVTec-AD_numpy/'
            # ).rsplit('.', 1)[0] + '.npy'
...
########################################################################################
    ...
    def _predict_dataloader(self, dataloader, prefix):
        ...
        with tqdm.tqdm(dataloader, desc="Inferring...", leave=False) as data_iterator:
            for data in data_iterator:
                ...
                _scores, _masks, _feats, _output = self._predict(image)
                scores.extend(_scores)
                # for score, mask, feat, is_anomaly in zip(_scores, _masks, _feats, data["is_anomaly"].numpy().tolist()):
                #     scores.append(score)
                #     masks.append(mask)
                #     features.append(feat)  # 添加特征到features列表
                # outputs.extend(_output)
        outputs=[]
        return scores, masks, features, labels_gt, masks_gt, outputs
    ...
    def _predict(self, images):
        ...
        with torch.no_grad():
            ...
            '''
        masks,output=[],[]
        # 修复：返回每个图像的patch分数数组，而不是标量列表
        # image_scores形状: (batchsize * num_patches,)
        scales = patch_shapes[0]
        num_patches_per_image = scales[0] * scales[1]
        
        # 将一维数组分割为每个图像的patch分数数组
        image_patch_scores = []
        for i in range(batchsize):
            start_idx = i * num_patches_per_image
            end_idx = (i + 1) * num_patches_per_image
            image_patch_scores.append(image_scores[start_idx:end_idx])
        return image_patch_scores, list(masks), list(features), list(output)
        #return list(image_scores), list(masks), list(features), list(output)
