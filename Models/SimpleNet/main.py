    ...
    for dataloader_count, dataloaders in enumerate(list_of_dataloaders):
        ...
        for i, SimpleNet in enumerate(simplenet_list):
            ...
            if not test:
                ...
                LOGGER.info("Testing models ({}/{})".format(i + 1, len(simplenet_list)))
                # 修复后的test方法
                pretrained_model_dir = os.path.join("CheckPoints", dataset_name)
                SimpleNet.set_model_dir(pretrained_model_dir, "")
                i_auroc, p_auroc, pro_auroc = SimpleNet.test(dataloaders["training"], dataloaders["testing"], save_segmentation_images)
...
def dataset(
    ...
    def get_dataloaders(seed):
        ...
        for subdataset in subdatasets:
            ...
            LOGGER.info(f"Dataset: test={len(test_dataset)}")
