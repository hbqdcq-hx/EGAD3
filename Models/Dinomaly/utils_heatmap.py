...
def evaluation_batch(model, dataloader, device, _class_=None, max_ratio=0, resize_mask=None):
    ...
    # 初始化详细数据容器
    prediction_scores, anomaly_maps, img_paths = [], [], []
    ...
    with torch.no_grad():
        for img, gt, label, img_path in dataloader:
            ...
            # 保存原始 anomaly_map 用于热力图
            anomaly_map_original = anomaly_map.clone()  # 保存原始异常图
            ...
            # 新增：保存详细数据
            anomaly_map_np = anomaly_map_original.cpu().numpy()  # 使用原始 anomaly_map
            for i in range(anomaly_map_np.shape[0]):
                # 提取异常图，确保是2D数组
                anomaly_maps.append(anomaly_map_np[i, 0])  # 2D 异常图 [H, W]
                img_paths.append(img_path[i])
                prediction_scores.append(sp_score[i].cpu().numpy())
