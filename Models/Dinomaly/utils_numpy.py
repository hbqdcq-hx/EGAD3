...
def evaluation_batch(model, dataloader, device, _class_=None, max_ratio=0, resize_mask=None):
    ...
    with torch.no_grad():
        ...
        for img, _,label, img_path in dataloader:
            img = img.to(device)
            output = model(img)
...
#############################################################################################TODO
            #en 2*(16,768,28,28)
            # 保存en和de特征图
            # img_path的长度应该等于batchsize=16

            for batch_idx, img_path_str in enumerate(img_path):
                # folder_path = img_path_str.replace(
                #     '/mnt/T4_2/xjj/FirstWorkData1/', 
                #     '/mnt/T4_1/xjj/Dinomaly/FirstWorkData_numpy/'
                # ).rsplit('.', 1)[0]
                
                folder_path = img_path_str.replace(
                    '/mnt/T38/bioinf/xjj/Datasets/MVTec-AD/', 
                    '/mnt/T4_1/xjj/Dinomaly/MVTec-AD_numpy/'
                ).rsplit('.', 1)[0]

                os.makedirs(folder_path, exist_ok=True)
                
                # 保存en特征图
                # en[0]是en_f, en[1]是en_a (形状都是(16,768,28,28))
                # batch_idx对应批次中的第几个样本
                np.save(os.path.join(folder_path, f"en_f_28x28.npy"), en[0][batch_idx].cpu().detach().numpy())
                np.save(os.path.join(folder_path, f"en_a_28x28.npy"), en[1][batch_idx].cpu().detach().numpy())
                
                # 保存de特征图
                # de[0]是de_f, de[1]是de_a (形状都是(16,768,28,28))
                np.save(os.path.join(folder_path, f"de_f_28x28.npy"), de[0][batch_idx].cpu().detach().numpy())
                np.save(os.path.join(folder_path, f"de_a_28x28.npy"), de[1][batch_idx].cpu().detach().numpy())
...
########################################################################################################################
        ...
        # 暂存默认指标值
        ap_sp, f1_sp = 0, 0
        auroc_sp,auroc_px, ap_px, f1_px, aupro_px = 0, 0, 0, 0,0
    ...
    # 返回10个值：原来的7个指标 + 预测分数 + 异常图 + 图像路径
    return auroc_sp, ap_sp, f1_sp, auroc_px, ap_px, f1_px, aupro_px, np.array(prediction_scores), anomaly_maps, img_paths
