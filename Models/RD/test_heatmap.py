...
def test(_class_, save_dir=None):
    ...
    auroc_sp, pr_list_sp = evaluation(encoder, bn, decoder, test_dataloader, device, _class_)
   
    # 获取所有 img_paths
    img_paths = test_data.img_paths
    nor_pr_list_sp = (pr_list_sp - np.min(pr_list_sp)) / (np.max(pr_list_sp) - np.min(pr_list_sp))
    ...
    with open(csv_path, 'w') as f:
        ...
        # 使用zip确保一一对应
        for name, score in zip(file_names, nor_pr_list_sp):
            f.write(f"{name},{score:.10f}\n")  # 使用逗号分隔的表格格式
    
    print(f"异常分数已保存到: {csv_path}")
    
    # 保存热力图
    save_heatmaps(encoder, bn, decoder, test_dataloader, device, file_names, save_dir, _class_)
...
def save_heatmaps(encoder, bn, decoder, dataloader, device, file_names, save_dir, _class_):
    """
    保存热力图到指定位置
    ...
    count = 0
    with torch.no_grad():
        for batch_idx, (img, label, _, original_size) in enumerate(dataloader):
            img = img.to(device)
            inputs = encoder(img)
            outputs = decoder(bn(inputs))
            
            # 计算异常图（使用evaluation函数中的方法，与第74行一致）
            ...
            # Get target size (height, width) - original_size is (width, height)
            orig_height = original_size[1].item()
            orig_width = original_size[0].item()
            ...
            ano_map = min_max_norm(upsampled_np)
